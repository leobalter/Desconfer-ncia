...
<section id="content" class="narrowcolumn">
    ...
    <article class="post" id="post-<?php the_ID(); ?>"><h2><?php the_title(); ?></h2>
        ...
        <div class="entry">
            <?php the_content('<p class="serif">' . __('Read the rest of this entry &raquo;', 'minimalism') . '</p>'); ?>                <?php wp_link_pages(array('before' => '<p><strong>' . __('Pages:', 'minimalism') . '</strong> ', 'after' => '</p>', 'next_or_number' => 'number')); ?>                <?php the_tags('<p>' . __('Tags:', 'minimalism') . ' ', ', ', '</p>'); ?>
            <p class="postmetadata alt">
                <small>
                    ...
                    /* $entry_datetime = abs(strtotime($post->post_date) - (60*120));
                                  $time_since = sprintf(__('%s ago', 'minimalism'), time_since($entry_datetime)); */ ?>
